...
/// Command for resetting a project directly
struct ResetCommand: AsyncParsableCommand {
    static let configuration = CommandConfiguration(
        commandName: "reset",
        abstract: "Reset a project (clean build, derived data, reinstall dependencies)"
    )

    @Argument(help: "Project name to reset")
    ...
    func run() async throws {
        ...
        do {
            ...
            let result = try await operationController.executeOperation(.reset, on: project)
